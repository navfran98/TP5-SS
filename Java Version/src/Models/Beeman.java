...
public class Beeman {
    ...
    public static Particle run(int index, Sylo sylo){
        ...
        // Forces
        Pair<Double, Double> currentForce = Force.calculateForce(p, sylo);
        Pair<Double, Double> prevForce = Force.calculateForce(prevp, sylo);
        ...
        Pair<Double, Double> newForce = Force.calculateForce(newp, sylo);
        
        double newVx= v0x + (1.0/3.0) * (newForce.first/p.mass) * sylo.dt + (5.0/6.0) * (currentForce.first/p.mass) * sylo.dt - (1.0/6.0) * (prevForce.first/p.mass) * sylo.dt;
        double newVy= v0y + (1.0/3.0) * (newForce.second/p.mass) * sylo.dt + (5.0/6.0) * (currentForce.second/p.mass) * sylo.dt - (1.0/6.0) * (prevForce.second/p.mass) * sylo.dt; 
